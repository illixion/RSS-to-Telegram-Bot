...
def parse_ss(url, min_price, max_price):
    ...
    if page.getcode() == 200:
        ...
        try:
            ...
            model = page_html.find("td", {"id": "tdo_127"}).string

            # GPU model filter
            model_p = model.replace(" ", "").lower()
            found = False
            if config.gpu_model is not None:
                for gpu in config.gpu_model:
                    if gpu in model_p:
                        found = True
                        msrp = config.gpu_model_to_msrp[gpu]
                        break
            if not found:
                return {"filter_ok": False}

            try:
                msrp_to_price_difference = (price_formatted - msrp) * 100 / msrp
            except ZeroDivisionError:
                msrp_to_price_difference = None

            memory = page_html.find("td", {"id": "tdo_41"}).string
            newused = page_html.find("td", {"id": "tdo_352"}).string
...
💵 {price} ({msrp_to_price_difference}% increase)"""
